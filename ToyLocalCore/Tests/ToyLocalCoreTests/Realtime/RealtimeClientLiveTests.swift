...
/// TOY_LOCAL_REALTIME_LIVE=1 [TOY_LOCAL_CLOUD_API_URL=http://127.0.0.1:8787]
/// TOY_LOCAL_REALTIME_FIXTURE=/path/to/16k-mono.wav swift test --filter RealtimeClientLiveTests
@Suite struct RealtimeClientLiveTests {
  private static var isEnabled: Bool {
    ProcessInfo.processInfo.environment["TOY_LOCAL_REALTIME_LIVE"] == "1"
  }

  @Test(.enabled(if: isEnabled)) func streamsFixtureAudioAndReceivesTranscript() async throws {
    let environment = ProcessInfo.processInfo.environment
    let baseURLString = environment["TOY_LOCAL_CLOUD_API_URL"] ?? "http://127.0.0.1:8787"
    let fixturePath = try #require(environment["TOY_LOCAL_REALTIME_FIXTURE"])
    let model = environment["TOY_LOCAL_REALTIME_MODEL"] ?? "nova-3"
