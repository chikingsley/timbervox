...
- [ ] Realtime step 2 — dictation integration: feed the capture engine's converted 16k float32 buffers into the client during recording, surface partials as a streaming preview, and select realtime cloud models from the catalog (re-add the Modes realtime toggle when this lands). [B]
- [ ] Worker: on `{"type":"close"}` the Durable Object sends Finalize/CloseStream upstream but closes the client socket immediately, so the provider's final Results after the flush can be lost — hold the client socket open briefly until the final arrives (the live test saw the tail of the utterance cut). [A]
...
- [ ] Retire the legacy JSON history mirror: transcripts are dual-written to the GRDB store AND the old JSON history (`TranscriptHistoryPersistence`); once the store is trusted in real use, the JSON side becomes read-never redundancy and should be removed (keep the one-time `importLegacy` migration). [A]
- [ ] Delete the AVAudioRecorder fallback once soak evidence shows it never fires: every recording logs `backend=capture-engine` or `backend=recorder-fallback`; if the fallback stays silent through a couple of weeks of real use, remove `RecordingClientLive+RecorderFallback.swift` and the recorder state. Same trigger retires the mic legacy numeric-ID resolution branch. [A]
