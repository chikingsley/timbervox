...
- [x] 2026-07-05 DEPLOYED: the worker runs at `https://toylocal.peacockery.studio` (custom domain, D1 migrated, all three provider secrets set) and the app defaults to it (`TOY_LOCAL_CLOUD_API_URL` still overrides for local dev). Verified live in production: realtime WebSocket session streamed fixture speech to Deepgram with complete finals, and the D1-backed jobs route answers correctly. WARNING: the deployed worker has NO auth yet — it is an open proxy to the provider API keys for anyone who finds the URL, which makes the auth work the next priority.
- [ ] Cloud auth + payments (DESIGN SETTLED 2026-07-05, RevenueCat decided; worker is currently an OPEN PROXY in production, so this is the top priority). Architecture: the app never talks to RevenueCat directly — it holds a generated `app_user_id` (Keychain) and talks only to our worker. Free tier: rate-limited `POST /v1/devices/register` mints a free-tier credential via the existing license machinery; the app sends it as the bearer token `ToyLocalCloudClient` already supports. Payments: RevenueCat Web Billing (hosted Stripe checkout at `pay.rev.cat/<token>/<app_user_id>`, one-time and subscription products, customer portal, no RC fee below $2.5k monthly tracked revenue then 1%); the Upgrade button opens the link in the browser and the post-purchase redirect returns via the `toylocal://` URL scheme. Entitlements: RC webhooks (HMAC-verified, idempotent by event id) mirror `app_user_id → pro` into D1, with `GET /v1/subscribers/{app_user_id}` (RC secret key as a worker secret) as the reconciliation read; the worker upgrades the device credential's tier. No RevenueCat SDK ships in the app — RC's docs don't cover Developer ID Mac apps for the SDK, and the worker needs the entitlement anyway to gate cloud usage. [A: webhook + registration + entitlement tests against wrangler dev; C: Chi does one real test purchase]
