...
export const getOwnedJob = (
  ...
  env.DB.prepare("SELECT * FROM jobs WHERE id = ? AND owner_user_id = ?")
    .bind(id, ownerUserId)
    .first<JobRow>();
...
export const createJob = async (
  ...
  await env.DB.prepare(
    `INSERT INTO jobs
      (id, kind, status, input_key, client_id, credential_id, owner_user_id,
       params_json, progress, created_at, updated_at, queued_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  )
    .bind(
      id,
