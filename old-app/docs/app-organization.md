...
Current status: stale proposal, not current implementation.
The current app uses `AppShellView` with a custom sidebar and the mounted tabs are Home, Modes, Configuration, Sound, Hot Mic, Model library, and History.
The `TabView` / `.sidebarAdaptable` material below is parked architecture research, not an active implementation plan.
Current work items live in `docs/TODO.md`; when this file disagrees with `docs/TODO.md` or `ActiveTab.swift`, treat this file as stale.

Original proposed layout: every page, what's on it, and where every existing control moves.
