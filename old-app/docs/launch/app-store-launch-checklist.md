...
- [x] Build with the required Xcode/SDK — verified 2026-07-08: archived with Xcode 26.x / macOS SDK 26.5, upload accepted. [agent]
- [ ] Decide Apple-silicon-only vs universal. Note: build 78 uploaded 2026-07-08 is UNIVERSAL (arm64 + x86_64) via the default export; if Chi picks ARM64-only, change the target's ARCHS before the next archive. [Chi confirms]
- [x] Distribution signing verified end-to-end 2026-07-08: automatic signing minted the Cloud Managed Apple Distribution cert + "Mac Team Store Provisioning Profile: com.chiejimofor.timbervox" at archive; package validated and uploaded. [agent]
...
- [x] App ID `com.chiejimofor.timbervox` registered (auto-created by automatic signing during the 2026-07-08 archive). [agent]
- [x] App record created by Chi 2026-07-08: name `TimberVox`, Apple ID `6788874335`, bundle ID + SKU `com.chiejimofor.timbervox`, 1.0 version row present. [Chi]
...
- [x] App Store Connect upload credentials verified locally: API key id `F0YBUEMFRDLI` (INDIVIDUAL key — REST JWTs need `sub: "user"` and no `iss` claim), issuer id `d1804d83-f266-43bc-8cda-edb51b2c2354`, matching ignored `.p8` under `Config/keys/`. Current app Apple ID: `6788874335` (the 2026-07-06 note referenced the deleted ArkVoiceOS record). [agent verified 2026-07-08]
...
- [x] Upload path proven end-to-end 2026-07-08: export → validate (VERIFY SUCCEEDED) → upload --wait; build 1.0 (78) is on App Store Connect, import-status VALID, APP_STORE_ELIGIBLE, non-exempt encryption = false, expires 2026-10-06 if unused. Do not submit for public App Review until the review metadata, privacy answers, and RevenueCat production key are ready. [agent]
