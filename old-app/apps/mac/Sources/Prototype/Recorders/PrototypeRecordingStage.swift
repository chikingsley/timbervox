...
private struct AudioLevelKey: EnvironmentKey {
  static let defaultValue: Double? = nil
}

extension EnvironmentValues {
  var protoAudioLevel: Double? {
    get { self[AudioLevelKey.self] }
    set { self[AudioLevelKey.self] = newValue }
  }
}
...
struct Waveform: View {
  ...
  var body: some View {
    TimelineView(.animation(minimumInterval: 1.0 / 30.0, paused: !active)) { context in
      ...
      let envelope = active ? (liveLevel ?? SpeechEnvelope.level(at: t)) : 0
...
struct PrototypeRecordingStage: View {
  ...
  @State private var mic = MicLevel()
...
struct OriginalCapsuleMock: View {
  ...
  var body: some View {
    TimelineView(.animation(minimumInterval: 1.0 / 30.0, paused: state != .recording && state != .streaming)) { context in
      let level = SpeechEnvelope.level(at: context.date.timeIntervalSinceReferenceDate)
