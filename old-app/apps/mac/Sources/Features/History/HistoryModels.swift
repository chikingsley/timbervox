...
enum HistoryDayFilter {
  static func options(for items: [HistoryItem]) -> [MenuOption<String?>] {
    let labels = orderedDayLabels(for: items)
    return [MenuOption(value: String?.none, label: "Any day", systemImage: "calendar")]
      + labels.map { MenuOption(value: $0, label: $0, systemImage: "calendar") }
  }
...
struct HistoryAppFilter: Hashable, Sendable {
  ...
  static func options(for items: [HistoryItem]) -> [MenuOption<HistoryAppFilter>] {
    ...
    return filters.map { MenuOption(value: $0, label: $0.label, systemImage: $0.systemImage) }
  }
}
