    ...
    background(fill, in: RoundedRectangle(cornerRadius: cornerRadius))
      .overlay(
        RoundedRectangle(cornerRadius: cornerRadius)
          .strokeBorder(Theme.borderStroke, lineWidth: 1)
      )
  }
}
...
struct HistoryHeaderActions: View {
  ...
  var body: some View {
    HStack(spacing: 12) {
      IconButton(systemName: "info.circle", tileSize: 22, hitSize: 28, help: "Recording details", action: openDetails)
      IconButton(
    ...
    HStack(alignment: .top, spacing: 12) {
      ...
      VStack(alignment: .leading, spacing: 7) {
        ...
        Text(item.preview)
          ...
          .lineLimit(2)
        HStack(spacing: 8) {
          KeyChip(item.app.rawValue)
          KeyChip(item.duration)
          KeyChip(item.mode)
        }
      }
    }
