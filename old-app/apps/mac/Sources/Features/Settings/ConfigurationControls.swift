          ...
          .overlay(
            RoundedRectangle(cornerRadius: 10)
              .strokeBorder(
                selected
                  ? Color.accentColor
                  : (hovering ? Color.primary.opacity(0.22) : Theme.borderStroke),
...
struct ConfigurationMenuRow<Value: Hashable & Sendable>: View {
  ...
  var body: some View {
    SettingsRow(icon: icon, title: title, hint: hint) {
      OptionMenu(
        selection: $selection,
        options: options
      )
...
struct ConfigurationHeaderPill: View {
  ...
  var body: some View {
    ...
    .frame(height: 28)
    .background(Theme.fieldSurface, in: RoundedRectangle(cornerRadius: Theme.fieldRadius))
  }
}
...
struct ConfigurationPermissionPill: View {
  ...
  var body: some View {
    ...
    .padding(.vertical, 4)
    .background(
      granted ? Theme.fieldSurface : Color(hex: Shadcn.orange400).opacity(0.14),
      in: Capsule()
    )
  }
}
