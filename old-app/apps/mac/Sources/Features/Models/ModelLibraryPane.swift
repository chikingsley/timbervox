...
struct ModelLibraryPane: View {
  ...
  private var headerContent: some View {
    SearchField(placeholder: "Search models...", text: $query)
      ...
      Button {
        ...
        HStack(spacing: ModelLibraryMetrics.supportHeaderSpacing) {
          ...
          Text(supportSectionModel?.title ?? ModelLibrarySectionID.supportAssets.title)
            .font(.system(size: ModelLibraryMetrics.sectionTitleSize, weight: .semibold))
            .foregroundStyle(.secondary)
          InfoHint("Support models are downloaded when a feature needs them. They are not normal dictation choices.")
          Spacer()
  ...
  private var storageFooter: some View {
    HStack(spacing: ModelLibraryMetrics.footerSpacing) {
      ...
      if downloadingCount > 0 {
        Text("\(downloadingCount) downloading")
          ...
          .background(
            Color(hex: Shadcn.orange400).opacity(ModelLibraryMetrics.downloadBadgeOpacity), in: RoundedRectangle(cornerRadius: Theme.chipRadius))
      }
      Spacer()
      Button {
    ...
    .overlay(alignment: .top) {
      Rectangle().fill(Theme.hairline).frame(height: ModelLibraryMetrics.hairlineHeight)
    }
  }
