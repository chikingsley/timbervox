...
private struct ModelLibraryFilterPanel: View {
  ...
  var body: some View {
    ...
    .background(popoverSurface, in: RoundedRectangle(cornerRadius: Metrics.cornerRadius))
    .overlay(
  ...
  var body: some View {
    Button(action: action) {
      HStack(spacing: Metrics.spacing) {
        ZStack {
          Circle()
            .strokeBorder(isSelected ? Theme.accentBlue : Color.secondary.opacity(0.45), lineWidth: Metrics.indicatorLineWidth)
            .background(Circle().fill(isSelected ? Theme.accentBlue : Color.clear))
          if isSelected {
      ...
      .padding(.horizontal, Metrics.horizontalPadding)
      .frame(height: Metrics.height)
      .background(RoundedRectangle(cornerRadius: Metrics.cornerRadius).fill(hovering ? Theme.hoverFill : Color.clear))
  ...
  @ViewBuilder private var selectionIndicator: some View {
    if row.entry.isSelectable {
      Image(systemName: isSelected ? "checkmark.circle.fill" : "circle")
        ...
        .foregroundStyle(isSelected ? Theme.accentBlue : Color.secondary.opacity(0.38))
