...
struct ModeDraft: Identifiable, Equatable {
  ...
  static func defaultMode(from settings: TimberVoxSettings, languageName: String) -> ModeDraft {
    ...
    return ModeDraft(
      ...
      voiceModel: voice,
      useRealtime: settings.realtimeTranscriptionEnabled,
...
struct ModeModelOption: Identifiable, Equatable {
  ...
  static let voiceModels: [ModeModelOption] =
    TranscriptionModelCatalog.userSelectableASR
    .filter { $0.capabilities.batch }
    .map { spec in
      ModeModelOption(
        id: spec.id,
        name: spec.displayName,
        provider: ProviderBrand(providerID: spec.provider),
        description: spec.runtime == .local
          ? "Runs on this Mac via \(spec.provider.rawValue)."
          : "Cloud model routed through TimberVox Cloud.",
        badge: "",
        supportsRealtime: spec.capabilities.realtime || RealtimeModelRouting.realtimeRouteID(forModelID: spec.id) != nil,
  ...
  static let languageModels: [ModeModelOption] = CloudLanguageModels.all.map { spec in
    ModeModelOption(
      id: spec.id,
      name: spec.displayName,
      provider: ProviderBrand(languageProviderID: spec.provider.rawValue),
