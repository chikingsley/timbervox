...
struct ModesModelMenu: View {
  ...
  init(
    selection: Binding<ModeModelOption>,
    models: [ModeModelOption],
    modelDownload: ModelDownloadStore? = nil,
    width: CGFloat = 190
  ) {
...
struct ModesModelPicker: View {
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 0) {
      SearchField(placeholder: "Search models", text: $searchText)
        .padding(10)
    ...
    .task {
      guard let modelDownload, modelDownload.availableModels.isEmpty else { return }
  ...
  private func modelButton(_ model: ModeModelOption) -> some View {
    ModesModelPickerButton(
      ...
      startDownload: { modelDownload?.downloadModel(model.id) },
      cancelDownload: { modelDownload?.cancelDownload() },
      delete: { modelDownload?.deleteModel(model.id) }
    )
  }

  private func downloadState(for model: ModeModelOption) -> ModelDownloadControlState {
    guard model.availability == .local else { return .cloud }
    if modelDownload?.downloadingModelName == model.id {
      return .downloading(modelDownload?.downloadProgress ?? 0)
    }
    if modelDownload?.availableModels.first(where: { $0.id == model.id })?.isDownloaded == true {
      return .downloaded
    }
...
struct ModesModelPickerButton: View {
  ...
  let isHovered: Bool
  let isFavorite: Bool
  let downloadState: ModelDownloadControlState
  let onSelect: () -> Void
    ...
    Button(action: onSelect) {
      ...
      .background(
        RoundedRectangle(cornerRadius: 6)
          .fill(isSelected ? Theme.accentGreen.opacity(0.12) : (isHovered ? Color.primary.opacity(0.07) : .clear))
      )
      .overlay {
        if isSelected {
          RoundedRectangle(cornerRadius: 6)
