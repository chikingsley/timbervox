...
struct FloatingPresentation: Identifiable {
  let id: AnyHashable
  let anchor: CGRect
...
@MainActor
final class FloatingLayerModel: ObservableObject {
  @Published var presentations: [FloatingPresentation] = []
  ...
  func present<Content: View>(
    ...
    let presentation = FloatingPresentation(
      id: id,
      anchor: anchor,
      placement: placement,
      spacing: spacing,
...
private struct FloatingLayerKey: EnvironmentKey {
  static let defaultValue: FloatingLayerModel? = nil
}

private struct FloatingCoordinateSpaceKey: EnvironmentKey {
  static let defaultValue = "FloatingLayer"
}

extension EnvironmentValues {
  var floatingLayer: FloatingLayerModel? {
    get { self[FloatingLayerKey.self] }
    set { self[FloatingLayerKey.self] = newValue }
  }

  var floatingCoordinateSpace: String {
    get { self[FloatingCoordinateSpaceKey.self] }
    set { self[FloatingCoordinateSpaceKey.self] = newValue }
  }
}

struct FloatingHost<Content: View>: View {
  @StateObject private var layer = FloatingLayerModel()
  ...
  private let coordinateSpaceName = "FloatingLayer"
    ...
    GeometryReader { proxy in
      ZStack(alignment: .topLeading) {
        content
          ...
          .environment(\.floatingLayer, layer)
          .environment(\.floatingCoordinateSpace, coordinateSpaceName)
        ...
        if Self.usesChildWindows {
          FloatingWindowBridge(layer: layer)
  ...
  private func floatingOrigin(
    for presentation: FloatingPresentation,
    measuredSize: CGSize,
    bounds: CGRect
  ) -> CGPoint {
    let inset: CGFloat = 8
...
extension View {
  func floatingAnchor(_ frame: Binding<CGRect>, in coordinateSpace: String) -> some View {
