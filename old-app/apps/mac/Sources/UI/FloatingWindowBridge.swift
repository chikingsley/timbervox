...
struct FloatingWindowBridge: NSViewRepresentable {
  @ObservedObject var layer: FloatingLayerModel

  func makeNSView(context: Context) -> FloatingBridgeView {
    let view = FloatingBridgeView()
    view.layerModel = layer
    return view
  }

  func updateNSView(_ nsView: FloatingBridgeView, context: Context) {
    nsView.layerModel = layer
    nsView.reconcile()
  }
}

final class FloatingBridgeView: NSView {
  var layerModel: FloatingLayerModel? {
    didSet {
  ...
  private func makePanel(for presentation: FloatingPresentation) -> NSPanel {
    let panel = NSPanel(
      contentRect: .zero,
  ...
  private func panelFrame(for presentation: FloatingPresentation, size: CGSize, window: NSWindow) -> CGRect {
