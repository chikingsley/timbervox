...
enum ModelDownloadControlState: Equatable {
  case downloaded
...
struct ModelDownloadControl: View {
  let state: ModelDownloadControlState
  var hovering = false
    ...
    case .downloaded:
      Button(action: delete) {
        Image(systemName: "trash")
          ...
          .foregroundStyle(hovering ? Theme.destructive.opacity(0.9) : Color.secondary.opacity(0.7))
  ...
  var body: some View {
    ZStack {
      ...
      Circle()
        ...
        .stroke(Theme.accentBlue, style: StrokeStyle(lineWidth: Metrics.lineWidth, lineCap: .round))
