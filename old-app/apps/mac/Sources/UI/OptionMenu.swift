...
struct MenuOption<Value: Hashable & Sendable>: Identifiable, Sendable {
  let value: Value
  let label: String
...
struct OptionMenu<Value: Hashable & Sendable>: View {
  @Binding var selection: Value
  let options: [MenuOption<Value>]
  var width: CGFloat = OptionMenuMetrics.pillWidth
  var panelWidth: CGFloat?
  var showsAllRows = false
  var selectedTint = Theme.accentGreen
  var onSelect: (Value) -> Void = { _ in }

  @State private var presentationID = UUID()
  @State private var anchorFrame: CGRect = .zero
  @Environment(\.floatingLayer) private var floatingLayer
  @Environment(\.floatingCoordinateSpace) private var coordinateSpace

  private var selectedOption: MenuOption<Value> {
  ...
  var body: some View {
    ...
    .buttonStyle(.plain)
    .floatingAnchor($anchorFrame, in: coordinateSpace)
  ...
  private var estimatedPanelHeight: CGFloat {
    OptionMenuMetrics.panelHeight(forRowCount: options.count, showsAllRows: showsAllRows)
  }
}

struct OptionValuePill: View {
  let text: String
  var systemImage: String?
  var width: CGFloat = OptionMenuMetrics.pillWidth
  var selectedTint = Theme.accentGreen
  ...
  var body: some View {
    ...
    .frame(width: width, height: OptionMenuMetrics.pillHeight, alignment: .leading)
    .background(.primary.opacity(0.06), in: RoundedRectangle(cornerRadius: 8))
  }
}

private struct OptionMenuPanel<Value: Hashable & Sendable>: View {
  let menuID: UUID
  @Binding var selection: Value
  let options: [MenuOption<Value>]
  let width: CGFloat
  ...
  private var needsScroll: Bool {
    !showsAllRows && options.count > OptionMenuMetrics.maxVisibleRows
  }
  ...
  var body: some View {
    Group {
      if needsScroll {
        ...
        .frame(height: OptionMenuMetrics.listHeight(forVisibleRows: OptionMenuMetrics.maxVisibleRows))
      } else {
        rowList
      }
    }
    .padding(OptionMenuMetrics.panelPadding)
    .frame(width: width)
    .background(popoverSurface, in: RoundedRectangle(cornerRadius: 9))
    .overlay(
  ...
  private var rowList: some View {
    VStack(spacing: OptionMenuMetrics.rowSpacing) {
...
#Preview("Long list caps at six rows") {
  OptionMenuPanel(
    ...
    ].map { MenuOption(value: $0, label: $0) },
    width: OptionMenuMetrics.pillWidth,
    showsAllRows: false,
...
private struct OptionMenuRow<Value: Hashable & Sendable>: View {
  let menuID: UUID
  let option: MenuOption<Value>
  let isSelected: Bool
  let selectedTint: Color
  let action: () -> Void
  ...
  var body: some View {
    Button(action: action) {
      ...
      .frame(height: OptionMenuMetrics.rowHeight)
      .background(
