    ...
    Button(action: beginRecording) {
      HStack(spacing: Self.keySpacing) {
        ForEach(keys.indices, id: \.self) { index in
          Keycap(keys[index], size: Self.keySize)
        }
      }
      .contentShape(Rectangle())
    }
  ...
  struct RecorderDemo: View {
    ...
    var body: some View {
      VStack(alignment: .trailing, spacing: 14) {
        ShortcutRecorder(keys: $chord, defaultKeys: ["⌥", "⇧", "K"])
        ShortcutRecorder(keys: $single, defaultKeys: ["⌥"])
        ShortcutRecorder(keys: $escape, defaultKeys: ["esc"])
        ShortcutRecorder(keys: $mouse)
      }
      .padding(24)
    }
  }
