...
final class SettingsStoreHotKeyCaptureTests: XCTestCase {
  ...
  func testPasteHotKeyResetLeavesShortcutUnset() {
    let store = makeStore()
    store.timberVoxSettings.pasteLastTranscriptHotkey = HotKey(key: .v, modifiers: [.option, .shift])

    store.resetPasteLastTranscriptHotkey()

    XCTAssertNil(store.timberVoxSettings.pasteLastTranscriptHotkey)
    XCTAssertTrue(store.defaultPasteLastTranscriptHotKeyKeys.isEmpty)
  }
