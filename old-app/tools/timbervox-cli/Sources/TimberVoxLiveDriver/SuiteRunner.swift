...
private let stateAssertionTimeout: TimeInterval = 8
private let stateAssertionPollInterval: TimeInterval = 0.25
...
struct SuiteRunner {
  ...
  private func run(step: SuiteStep, app: AppControl) throws {
    ...
    case .assertState(let expectation):
      try assertStateEventually(app: app, expectation: expectation)
    }
  }

  private func assertStateEventually(app: AppControl, expectation: StateExpectation) throws {
    let deadline = Date().addingTimeInterval(stateAssertionTimeout)
    var lastError: Error?
    ...
    while Date() < deadline {
      ...
      Thread.sleep(forTimeInterval: stateAssertionPollInterval)
    }

    throw lastError ?? DriverError("Timed out waiting for expected debug state")
  }
