...
private enum ASC {
  ...
  // No default: the old ArkVoiceOS record (6787965139) was deleted; set this
  // once the TimberVox app record exists.
  static var appleID: String? { env("APP_STORE_CONNECT_APPLE_ID") }
...
struct AppStoreCommand: ParsableCommand {
  ...
  struct ListAppsCommand: ParsableCommand {
    ...
    func run() throws {
      try requireFile(ASC.p8Path, "Set APP_STORE_CONNECT_P8_PATH to the matching .p8 file.")
      var arguments = ["altool", "--list-apps"]
      if let appleID = ASC.appleID {
        arguments += ["--filter-apple-id", appleID]
      }
      try exec("/usr/bin/xcrun", arguments + ASC.authArgs + ["--output-format", "json"])
    }
  }
  ...
  struct AppcastCommand: ParsableCommand {
    static let configuration = CommandConfiguration(
      commandName: "appcast",
      abstract: "Generate the Sparkle appcast (wraps the vendored generate_appcast tool)."
    )
    @Argument(
      parsing: .captureForPassthrough,
      help: "Arguments passed to generate_appcast (e.g. the updates directory)."
