...
struct AppControl {
  ...
  func resetTCC(services: [TCCService]) {
    let bundleIdentifiers = bundleIdentifiersForTCCReset()
    for service in services {
      for bundleIdentifier in bundleIdentifiers {
        print("Resetting \(service.rawValue) for \(bundleIdentifier)")
        _ = try? runProcess(
          "/usr/bin/tccutil",
  ...
  private func bundleIdentifiersForTCCReset() -> [String] {
    var identifiers = [target.bundleIdentifier]
    if let appBundleIdentifier = try? appBundleURL().appBundleIdentifier {
      identifiers.append(appBundleIdentifier)
    }
    identifiers.append(contentsOf: target.legacyTCCBundleIdentifiers)
...
private extension URL {
  var appBundleIdentifier: String? {
    Bundle(url: self)?.bundleIdentifier
  }
}
