...
- Captured the app-folder, debug/diagnostics, Clients, Services, Features/UI, and State/Stores architecture audit in the TODO work list before any folder reorganization.
- Captured Chi's rejection of the current shortcuts, Super Fast Mode, update toggles, and settings organization in the TODO work list before any more UI wiring.
- Moved Hot Mic controls into a dedicated colored-card Configuration section, removed the separate Hot Mic sidebar pane, and removed the duplicate Home Hot Mic quick action.
- Surfaced all wired shortcut recorders in Configuration and made Paste Last Transcript unset by default instead of binding a concrete key.
- Restored the Home Start Recording quick action by wiring it to real app state, and marked the app-organization spec as stale architecture research instead of current implementation.
- Restored the Modes realtime toggle for realtime-capable voice models and wired it to the realtime transcription startup path.
- Removed unused MarkdownUI and swift-log package dependencies, and updated the License prototype so an active Pro license hides the purchase controls, discount code, and promo card.
- Fixed the Debug live driver's TCC reset/quit identity to match the built TimberVox bundle and clear the legacy debug bundle id, made live state assertions wait for async launch permission state, aligned the permission-onboarding suite with the current required-permissions UI, updated the Notch prototype for the modern SwiftUI `onChange` API, and repaired the cloud catalog test resolver for the current monorepo paths.
- Renamed shared UI primitives from `TL*` names to plain component names and centralized the header microphone menu device binding/options in `HeaderMicrophoneMenu`.
...
- Ported the prototype shell into the app: `AppShellView` with the custom sidebar (Home / Modes / Settings / History / Pro card) replaced the TabView container, the prototype UI kit was promoted to `TimberVox/UI/`, and the main window is now hidden-title/transparent-titlebar with a fixed 820pt width, vertical resize, and a minimize button that hides while the sidebar rail is collapsed.
