  ...
  func testCloudTranscriptionModelIDsExistInServerRoutes() throws {
    ...
    for model in TranscriptionModelCatalog.cloud {
      XCTAssertTrue(
        ...
        "\(model.id) missing from services/timbervox-api/src/ai/model-routes.ts"
      )
    }
  }
...
private func readCloudflareModelRoutes(filePath: String = #filePath) throws -> String {
  ...
  let routeFile = root.appendingPathComponent("services/timbervox-api/src/ai/model-routes.ts")
...
private func repositoryRoot(from fileURL: URL) throws -> URL {
  ...
  while directory.path != "/" {
    let routeFile = directory.appendingPathComponent("services/timbervox-api/src/ai/model-routes.ts")
    let packageFile = directory.appendingPathComponent("packages/timbervox-core/Package.swift")
