...
- Design tokens only: `Theme` + `Shadcn` named steps. No raw hex or opacity literals in panes.
- Shared UI primitives under apps/mac/Sources/UI/ use plain descriptive names without app-specific prefixes; group multiple declarations only when the file owns one real component family.
- `SettingsCard` interleaves dividers automatically; rows are `SettingsRow`/`SettingsToggleRow`; dropdowns are `OptionMenu` popovers (segmented controls excepted); never system Menu/Picker in panes.
- Dropdown panels show a bounded number of rows then scroll; the bound lives INSIDE the component.
- Shortcut recorder state machine lives in `ShortcutRecorder`; real capture goes through `SettingsStore` capture modes per docs/hotkey-semantics.md.
- Geometry plumbing uses `onGeometryChange` ONLY (GeometryReader + preference keys silently broke anchor delivery once already).
- Popovers require `FloatingHost`; every pane preview wraps in it.
- Headers are page-owned; no pane name in the header; empty header slot shows the microphone pill.
- The shipping app mounts ONLY wired features — no feature flags, no conditionals. Unfinished feature surfaces live in `Prototype/` (worked on via #Preview, never mounted in the app shell); graduating a feature = wire it for real, move it into Features/, and mount it. Removed-but-planned controls keep their restore conditions in docs/TODO.md.

## Resources (use them; do not re-derive)

- docs/README.md — the map of every doc. docs/TODO.md — the work list (dead/unwired UI is tracked there, sections "Dead controls — …"). docs/app-organization.md — the IA spec. docs/hotkey-semantics.md — hotkey engine behavior. docs/recorders/ — parked recorder HUD spec. docs/archive/ — history.
- Apple keys in Config/keys/ (gitignored; filenames embed Apple's key IDs — NEVER rename, tooling derives the key ID from the name). This list is authoritative — do not ask which key is which: `ApiKey_F0YBUEMFRDLI.p8` = App Store Connect API key (works with altool; raw REST JWTs need `sub: "user"` and NO `iss` claim — it is an individual key). `SubscriptionKey_5T4S8JA5P5.p8` = StoreKit In-App Purchase key (for RevenueCat). `AuthKey_WK85KTTVHF.p8` = a separate Apple key (APNs/Sign-in class — NOT for App Store uploads). `development.cer` = development signing certificate. ASC app record: TimberVox, Apple ID 6788874335, bundle com.chiejimofor.timbervox.
