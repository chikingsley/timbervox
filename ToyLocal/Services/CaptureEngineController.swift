...
private let captureLogger = ToyLocalLog.recording
...
private struct CaptureEngineConstants {
...
enum CaptureRecordingMode: String {
  ...
  var preRollDuration: TimeInterval {
    ...
    case .standard:
      0
    case .superFast:
      CaptureEngineConstants.defaultPreRollDuration
    }
  }
...
final class CaptureEngineController: @unchecked Sendable {
  ...
  private let processingQueue = DispatchQueue(label: "com.chiejimofor.toylocal.CaptureEngine")
  private let meterContinuation: AsyncStream<Meter>.Continuation
  private let ringBuffer = FloatRingBuffer(
    capacity: Int(CaptureEngineConstants.sampleRate * CaptureEngineConstants.ringBufferDuration)
  )
  ...
  init(
    ...
    guard
      let format = AVAudioFormat(
        ...
        sampleRate: CaptureEngineConstants.sampleRate,
        channels: 1,
        interleaved: false
      )
    else {
      preconditionFailure("The fixed capture engine PCM format is always constructible")
    }
  ...
  var stopTimingEstimate: StopTimingEstimate {
    processingQueue.sync {
      ...
      let gracePeriod = min(
        max(
          observedCadence > 0
            ? observedCadence + CaptureEngineConstants.stopGraceSafetyMargin
            : CaptureEngineConstants.fallbackStopGracePeriod,
          CaptureEngineConstants.minimumStopGracePeriod
        ),
        CaptureEngineConstants.maximumStopGracePeriod
      )
  ...
  func startIfNeeded(reason: String = "unknown", keepWarmBuffer: Bool = false) throws {
    ...
    if engine?.isRunning == true {
      captureLogger.debug("Capture engine already armed reason=\(reason)")
      return
    }
    ...
    inputNode.installTap(
      onBus: 0,
      bufferSize: CaptureEngineConstants.tapBufferSize,
      format: inputFormat
    ...
    let armedDetails = "sampleRate=\(String(format: "%.0f", inputFormat.sampleRate))Hz channels=\(inputFormat.channelCount)"
    let bufferDetails =
      "ringBuffer=\(String(format: "%.2f", CaptureEngineConstants.ringBufferDuration))s "
      + "defaultPreRoll=\(String(format: "%.2f", CaptureEngineConstants.defaultPreRollDuration))s"
    captureLogger.notice("Capture engine armed reason=\(reason) \(armedDetails) \(bufferDetails)")
  }

  func stop(reason: String = "unknown") {
    if engine != nil {
      captureLogger.notice("Capture engine stopped reason=\(reason)")
    }
  ...
  private func handleConfigurationChange() {
    captureLogger.notice("Capture engine configuration changed")
      ...
      do {
        let file = try AVAudioFile(
          ...
          settings: [
            ...
            AVSampleRateKey: CaptureEngineConstants.sampleRate,
        ...
        let preRollFrameCount = Int(preRollDuration * CaptureEngineConstants.sampleRate)
        let preRollSamples = ringBuffer.recentSamples(count: preRollFrameCount)
        let prependedDuration = Double(preRollSamples.count) / CaptureEngineConstants.sampleRate
        if !preRollSamples.isEmpty {
          try write(samples: preRollSamples, to: file)
        }

        let openedDetails =
          "prepended=\(String(format: "%.3f", prependedDuration))s "
          + "requestedPreRoll=\(String(format: "%.3f", preRollDuration))s"
        captureLogger.notice("Capture engine recording file opened \(openedDetails)")
  ...
  private func process(_ buffer: AVAudioPCMBuffer) {
    ...
    if !recording.didLogFirstBuffer {
      ...
      let firstBufferDetails =
        "latency=\(String(format: "%.3f", timeToFirstBuffer))s "
        + "prepended=\(String(format: "%.3f", recording.prependedDuration))s frames=\(sampleCount)"
      captureLogger.notice("Capture engine first buffer \(firstBufferDetails)")
    ...
    do {
      ...
      captureLogger.error("Failed to write capture engine audio: \(error.localizedDescription)")
      activeRecording = nil
    }
  }
...
extension CaptureEngineController {
  private func convert(_ inputBuffer: AVAudioPCMBuffer) -> AVAudioPCMBuffer? {
    ...
    if let error {
      captureLogger.error("Failed to convert capture engine audio: \(error.localizedDescription)")
      return nil
    }
  ...
  private func appendRecentMetric(_ value: TimeInterval, to metrics: inout [TimeInterval]) {
    ...
    if metrics.count > CaptureEngineConstants.callbackTimingWindowSize {
      metrics.removeFirst(metrics.count - CaptureEngineConstants.callbackTimingWindowSize)
    }
  }
}
