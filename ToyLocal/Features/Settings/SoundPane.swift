...
struct SoundPane: View {
  ...
  private var microphoneSelection: Binding<String> {
    Binding(
      get: { store.toyLocalSettings.selectedMicrophoneID ?? Self.systemDefaultMicrophoneID },
      set: { selectedID in
        store.toyLocalSettings.selectedMicrophoneID = selectedID == Self.systemDefaultMicrophoneID ? nil : selectedID
      }
    )
  }
  ...
  private var playbackRow: some View {
    TLSettingsRow(
      ...
      TLOptionMenu(
        selection: $store.toyLocalSettings.recordingAudioBehavior,
  ...
  private var soundEffectsRow: some View {
    TLSettingsRow(title: "Sound effects") {
      Picker(
        "",
        selection: Binding(
          get: { store.toyLocalSettings.soundEffectsStyle },
  ...
  private var volumeRow: some View {
    TLSettingsRow(title: "Volume") {
      HStack(spacing: SoundPaneMetrics.volumeIconSpacing) {
        ...
        Slider(
          value: $store.toyLocalSettings.soundEffectsVolume,
