...
actor ParakeetClient {
  ...
  private let logger = ToyLocalLog.parakeet
  private let vendorDirs = [
    // Our app-specific cache path convention (under XDG or com.chiejimofor.toylocal/cache)
    "fluidaudio/Models",
    "FluidAudio/Models",
  ...
  private func candidateRoots() -> [URL] {
    ...
    let appCache = appSupport?.appendingPathComponent("com.chiejimofor.toylocal/cache", isDirectory: true)
...
actor ParakeetClient {
  ...
  func ensureLoaded(modelName: String, progress: @Sendable @escaping (Progress) -> Void) async throws {
    throw NSError(
      domain: "Parakeet",
      code: -2,
      userInfo: [NSLocalizedDescriptionKey: "Parakeet support not linked. Add Swift Package: https://github.com/FluidInference/FluidAudio.git and link FluidAudio to ToyLocal."]
    )
  }
