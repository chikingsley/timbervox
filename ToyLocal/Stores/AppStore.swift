...
final class AppStore {
  ...
  private func prewarmSelectedModelIfNeeded(_ modelID: String) {
    ...
    guard settingsManager.settings.localModelPrewarmEnabled else {
      ToyLocalLog.transcription.info("Skipping selected model prewarm because local model prewarm is disabled.")
      return
    }
    guard let model = TranscriptionModelCatalog.model(id: modelID) else {
      ToyLocalLog.transcription.notice("Skipping selected model prewarm; model is not in catalog: \(modelID)")
      return
    }
    guard model.runtime == .local,
      ...
      ToyLocalLog.transcription.info("Skipping selected model prewarm; model is not local batch ASR: \(modelID)")
      return
    }

    selectedModelPrewarmTask = Task { [weak self] in
      guard let self else { return }
      ToyLocalLog.transcription.notice("Prewarming selected local ASR model=\(modelID)")
      do {
        try await self.transcriptionClient.downloadAndLoadModel(variant: modelID) { _ in }
        guard !Task.isCancelled else { return }
        ToyLocalLog.transcription.notice("Selected local ASR model is warm model=\(modelID)")
      } catch {
        guard !Task.isCancelled else { return }
        ToyLocalLog.transcription.error("Selected local ASR prewarm failed model=\(modelID) error=\(error.localizedDescription)")
      }
    }
  }
