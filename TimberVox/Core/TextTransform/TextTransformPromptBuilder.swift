...
public enum TextTransformPromptBuilder {
  ...
  private static func templateValues(
    ...
    if contextOptions.includeScreenContext,
      let screenText = nonEmpty(context.application?.screenText)
    {
      values["screen_context"] = "SCREEN TEXT CONTEXT:\n\(screenText)"
    }
    ...
    let normalizedLines = text.split(separator: "\n", omittingEmptySubsequences: false)
      .reduce(into: [String]()) { lines, line in
        ...
        if value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
          let lastLineIsEmpty = lines.last?.isEmpty ?? false
          if !lastLineIsEmpty {
            lines.append("")
          }
        } else {
